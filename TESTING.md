...
- With `cwd` distinct from home, leave an old `~/langsmith-plugins.json` enabled with credentials, disabled, or malformed: it must have no effect when `~/.langsmith-plugins.json` is absent or present. The hidden home file must be honored without any fallback to the old name. When `cwd` equals home, verify visible `langsmith-plugins.json` and hidden `.langsmith-plugins.json` coexist: the visible file remains the higher-priority project source, with missing fields falling through to the hidden home file (after user `.claude`).
- `src/config.test.ts` exercises live four-source files and exhaustive five-layer boolean precedence plus existing environment parsers (including empty strings, arrays, and legacy SDK replica tuples), ignored old home files, and coincident home/cwd paths. `src/hook-init.test.ts` checks file-only credentials and replica-only routing still require master enablement, including hidden home credentials with an ignored old home file.
