  ...
  await atomicUpdateState(config.stateFilePath, (state) => {
    ...
    return {
      ...
      [input.session_id]: {
        ...
        compaction_tracing: resolveTurnTracingMode(config.stateFilePath, input.session_id, input.trigger === "manual" ? void 0 : sessionState.current_turn_tracing, input.trigger !== "manual" && sessionState.current_turn_run_id ? sessionState.open_turns?.[sessionState.current_turn_run_id]?.tracing : void 0)
      }
    };
  });
