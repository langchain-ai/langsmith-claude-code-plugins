...
function initLogger(debug2) {
  debugEnabled = debug2;
  mkdirSync3(dirname2(LOG_FILE), { recursive: true });
}
...
async function main() {
  ...
  const tracing = resolveTurnTracingMode(config.stateFilePath, input.session_id, sessionState.tool_tracing_modes?.[input.tool_use_id], sessionState.current_turn_tracing, sessionState.current_turn_run_id ? sessionState.open_turns?.[sessionState.current_turn_run_id]?.tracing : void 0);
  ...
  await atomicUpdateState(config.stateFilePath, (freshState) => {
    ...
    if (agentId || workflow) {
      ...
      const launchingTurn = {
        tracing: resolveTurnTracingMode(config.stateFilePath, input.session_id, sessionState.current_turn_tracing, sessionState.current_turn_run_id ? sessionState.open_turns?.[sessionState.current_turn_run_id]?.tracing : void 0),
        run_id: parentRunId,
        trace_id: traceId,
        dotted_order: parentDottedOrder,
