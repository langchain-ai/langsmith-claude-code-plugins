...
function staleLock(path3) {
  try {
    if (Date.now() - statSync2(path3).mtimeMs <= LOCK_STALE_MS)
      return false;
    try {
      const record = JSON.parse(readFileSync(path3, "utf-8"));
...
function releaseLock(stateFilePath, lock) {
  const path3 = lockPath(stateFilePath);
  try {
    const current = JSON.parse(readFileSync(path3, "utf-8"));
...
// dist/transcript.js
import { readFileSync as readFileSync4, statSync as statSync4, fstatSync, openSync as openSync2, readSync, closeSync as closeSync2 } from "node:fs";
